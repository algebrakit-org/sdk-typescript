...
function verifyPenaltyAggregation(response: SessionScoreResponse): void {
  ...
  response.questions?.forEach(question => {
    ...
    if (qPenalties) {
      ...
      console.log(`   Question ${question.id} aggregation: ${hintsMatch && errorsMatch && penaltyMatch ? 'OK' : 'MISMATCH'}`);
      if (!hintsMatch) console.log(`     Hints mismatch: ${qPenalties.hintsRequested} vs ${questionHints}`);
      if (!errorsMatch) console.log(`     Errors mismatch: ${qPenalties.mathErrors} vs ${questionErrors}`);
      if (!penaltyMatch) console.log(`     Penalty mismatch: ${qPenalties.marksPenalty} vs ${questionPenalty}`);
    }
  });
  ...
  if (exercisePenalties) {
    ...
    console.log(`   Exercise level aggregation: ${hintsMatch && errorsMatch && penaltyMatch ? 'OK' : 'MISMATCH'}`);
    if (!hintsMatch) console.log(`     Hints mismatch: ${exercisePenalties.hintsRequested} vs ${totalHintsFromQuestions}`);
    if (!errorsMatch) console.log(`     Errors mismatch: ${exercisePenalties.mathErrors} vs ${totalErrorsFromQuestions}`);
    if (!penaltyMatch) console.log(`     Penalty mismatch: ${exercisePenalties.marksPenalty} vs ${totalPenaltyFromQuestions}`);
  } else {
    console.log('   No penalties at exercise level (might be a new session)');
  }
}
